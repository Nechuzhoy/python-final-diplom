...
class RegisterAccount(generics.CreateAPIView):
    ...
    """
    queryset = User.objects.all
    serializer_class = RegesterSerializer


class ConfirmAccount(generics.CreateAPIView):
    queryset = ConfirmEmailToken.objects.all()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if {'key'}.issubset(request.data):

            token = ConfirmEmailToken.objects.filter(key=request.data['key']).first()
            if token:
...
class PartnerUpdate(generics.ListCreateAPIView):
...
class PartnerState(generics.ListCreateAPIView):
    queryset = Shop.objects.all()
    serializer_class = PartnerUpdateSerializer
    permission_classes = [IsAuthenticated, IsUserOrReadOnly]
